...
try:
    ...
    # OAuth libraries not available, login functionality will be disabled
    OAUTH_AVAILABLE = False
    print("Warning: OAuth libraries not available. Login functionality disabled.")
...
try:
    ...
    print("Warning: Beaker not available. Session management disabled.")
...
def home():
    ...
    if SESSIONS_AVAILABLE:
        ...
        email = session.get('email', None) if session else None
    else:
        email = None
    email = None
...
if OAUTH_AVAILABLE:
    ...
    # Provide stub routes when OAuth is not available
    @app.route('/login', method='GET')
    def loginRedirect():
        return error_page("OAuth not configured - login unavailable", 503)
